...
module.exports = Control.extend({
  setup: function(el, options) {
    ...
    el = $(el);
    if (
      el.attr("id") === "versions" && el[0].nodeName.toLowerCase() === "select"
    ) {
      container = el;
    } else {
  ...
  getVersionSelectText: function(version) {
    return pageConfig.versionsSelectText
      ? pageConfig.versionsSelectText.replace(
          /<%=\s*version\s*%>/,
          "" + version
        )
      : version;
  ...
  getVersionedParentPath: function(version) {
    var path = (this.docConfig.versionDest || "./<%= version %>/<%= name %>")
      .replace(/<%=\s*version\s*%>/, "" + version)
      .replace(/<%=\s*name\s*%>/, "" + pageConfig.project.name);
    return dirname(path);
  },
  getDefaultParentPath: function() {
    var path = (this.docConfig.defaultDest || "./<%= name %>").replace(
      /<%=\s*name\s*%>/,
      "" + pageConfig.project.name
    );
    return dirname(path);
  },
  change: function(el, ev) {
    ...
    if (isVersioned && isNewCurrentVersion) {
      ...
      var toDocumentJSON = steal.joinURIs(
      ...
      var versioned = combine(
        toDocumentJSON,
        this.getVersionedParentPath(newVersion)
      );
